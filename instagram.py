...
class Engine(object):
    def __init__(self, emad_imt, threads, passlist_path, is_color):
        self.resume = False
        self.is_alive = True
        self.threads = threads
        self.username = emad_imt
        self.passlist_path = passlist_path
        self.display = Display(is_color=is_color)
        self.bruter = Bruter(emad_imt, threads, passlist_path)
...
def args():
    args = ArgumentParser()
    args.add_argument("-u", "--emad_imt", help="email or username")
    ...
    if not (prune_db or stats or proxylist) and not (emad_imt and passlist):
        args.print_help()
        exit()
...
def main():
    ...
    if prune_db > 0 or stats:
        ...
        if username and passlist and total_proxies:
            ...
            Engine(
                emad_imt, modes[mode], passlist, not arguments.color
            ).start()
