...
def args():
    ...
    parser.add_argument("-px", "--proxylist", help="proxy list")
    parser.add_argument(
        "--prune",
        default=-1,
        type=valid_float,
...
def display_database_stats():
    ...
    min_score = round(data["min"], place)
    max_score = round(data["max"], place)
    total = data["total"]
    health = (
        "Dead"
        if avg == 0
    ...
    print(
        f"Q1: {q1} :: Avg Score: {avg}  ::  Min Score: {min_score}  ::  Max Score: {max_score}"
    )
    time.sleep(0.65)
...
def main():
    ...
    prune_db = prune > 0
    username = "irteza_tariq14"

    if prune_db or stats:
        if prune_db:
